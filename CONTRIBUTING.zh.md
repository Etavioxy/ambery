...
- 行为改动带测试：Rust 单测在模块内，前端测试进 `packages/apps/test/*.test.ts`。
- 提交与注释不出现内部项目/示例名——仓库是公开的。

## 代码规则

- 前端读走 store（`packages/apps/src/store.ts`），写走 action 层（`packages/apps/src/tauri_runtime_actions.ts`）；不散落 `invoke`。
