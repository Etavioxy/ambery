...
> 概念定义见 concepts.md §Window（宿主的物理容器）。本文档定壳形态。
>
...
- 每个窗口加载 `index.html`，`main.ts` 按窗口 label 路由到 `entry/pet.ts` / `entry/menu.ts` / `entry/chat.ts` / `entry/shelf.ts` / `windows/card-window.ts`
