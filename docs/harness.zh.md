...
> 概念定义见 [concepts.md](../concepts.md) 的 Harness 及子概念。本文档定数据模型、注入规则、触发模型与 JSONL 存储格式。
...
6. **diff 事件化**：实例注册/状态翻转的簿记事件不走 Queue——走 Event Buffer 静默附带；LLM 从 Context 中的事件流重建全景，不按轮注入快照。

## Event Buffer 规则（concepts §Event Buffer）
...
- Autonomy 状态记录（type=autonomy）每轮一条也写 context.jsonl；装配请求时取最新一条挂请求末端（concepts §Autonomy / docs/storage.md）。
