...
开发期热更新迭代：把 vite dev server 固定到 `tauri.conf.json` 的 `devUrl` 端口（127.0.0.1:5174）——`cd packages/apps && npm run dev -- --port 5174 --strictPort`——再在 `packages/apps/tauri/` 下跑 `AMBERY_PORT=47602 npx tauri dev`（壳内嵌自己的 core server，`AMBERY_PORT` 必须避开独立后端）。
