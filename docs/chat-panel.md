...
> Concept definitions are in concepts.md §Chat Panel. This document specifies invocation/dismissal, layout, and message rendering rules.
...
- **Invoke/Dismiss**: right-click toggle on the View (`chat:toggle`) — concepts §Chat Panel "invoked via View right-click"; when the panel is open, right-clicking again closes it. The pet stays in place (no snapping teleport; docs/view.md §Gestures and Chat invocation).
...
- Conversation history is read from Context (concepts §Chat Panel); the panel is a **view projection** of Context and holds no data of its own.
- Of the four Context roles (concepts §Context), the panel renders only the content of `user` and `assistant`; `system` (event messages) and `tool` (execution results) are runtime messages and do not pollute the conversation view (design decision).
