...
The Config domain (concepts §Config, alongside the Storage domain) consists of `config.json` and `AGENTS.md`; the on-disk directory is determined by `config_root()` in `core/src/paths.rs`.
