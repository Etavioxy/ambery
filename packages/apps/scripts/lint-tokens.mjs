// 设计 token 守卫（node scripts/lint-tokens.mjs）：
// 1. styles/ 各 css 的 :root token 表之外禁止裸色值（#hex / rgb()/rgba()），白名单仅 #fff；
// 2. 所有 var(--ov-*) 引用必须在 :root 表内有定义；
// 3. src 业务 TS（windows/components/view/autonomy/pet-size）禁止裸色值（debug 面板豁免）。
// 退出码非零 = 违规（列出全部行号）。

import { readFileSync, readdirSync, statSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";

const srcDir = join(dirname(fileURLToPath(import.meta.url)), "../src");
// 样式层：token 表（tokens.css）与窗口/组件规则（index.css）分家后，两者都受同一条纪律约束
const cssFiles = ["styles/tokens.css", "styles/index.css"];

let failures = 0;
const fail = (msg) => {
  console.error(`FAIL: ${msg}`);
  failures++;
};

const COLOR_RE = /#[0-9a-fA-F]{3,8}\b|\brgba?\(/g;
/** 色值判别：hex 须含数字（排除 #face 等选择器/注释引用）；#fff 白名单单列 */
const isColor = (m) => (m.startsWith("#") ? /\d/.test(m) || m === "#fff" : true);
const VAR_RE = /var\((--ov-[a-z0-9-]+)\)/g;
const defined = new Set();

for (const rel of cssFiles) {
  const lines = readFileSync(join(srcDir, rel), "utf8").split("\n");
  // :root 块范围（每份样式表最多一张 token 表）
  const rootStart = lines.findIndex((l) => l.trim().startsWith(":root"));
  let rootEnd = -1;
  if (rootStart >= 0) {
    for (let i = rootStart, depth = 0; i < lines.length; i++) {
      depth += (lines[i].match(/\{/g) ?? []).length - (lines[i].match(/\}/g) ?? []).length;
      if (depth === 0 && i > rootStart) {
        rootEnd = i;
        break;
      }
    }
    for (let i = rootStart; i <= rootEnd; i++) {
      for (const m of lines[i].matchAll(/(--ov-[a-z0-9-]+)\s*:/g)) defined.add(m[1]);
    }
  }

  for (let i = 0; i < lines.length; i++) {
    if (rootStart >= 0 && i >= rootStart && i <= rootEnd) continue;
    const line = lines[i];
    for (const m of line.matchAll(COLOR_RE)) {
      if (m[0] === "#fff") continue; // 白名单：danger 底上的固定对比白
      if (!isColor(m[0])) continue;
      fail(`${rel}:${i + 1} 裸色值 ${m[0]} — ${line.trim().slice(0, 80)}`);
    }
    for (const m of line.matchAll(VAR_RE)) {
      if (!defined.has(m[1])) fail(`${rel}:${i + 1} 引用未定义 token ${m[1]}`);
    }
  }
}

if (defined.size === 0) fail("找不到 :root token 表（styles/tokens.css）");

// TS 侧裸色值（递归收集，豁免 debug 面板与 scripts）
const tsFiles = [];
(function walk(dir) {
  for (const name of readdirSync(dir)) {
    const p = join(dir, name);
    if (statSync(p).isDirectory()) walk(p);
    else if (p.endsWith(".ts")) tsFiles.push(p);
  }
})(srcDir);
for (const f of tsFiles) {
  if (f.includes("debug-vite-panel")) continue; // debug 专用，豁免
  const content = readFileSync(f, "utf8").split("\n");
  for (let i = 0; i < content.length; i++) {
    const t = content[i].trim();
    if (t.startsWith("//") || t.startsWith("*") || t.startsWith("/*")) continue; // 注释豁免
    for (const m of content[i].matchAll(COLOR_RE)) {
      if (!isColor(m[0])) continue;
      fail(`${f.split(/[\\/]src[\\/]/)[1]}:${i + 1} TS 裸色值 ${m[0]} — ${content[i].trim().slice(0, 80)}`);
    }
  }
}

// 4. 类名里的任意值只允许引用 token：bg-[#fff] / p-[12px] 违规，bg-[var(--ov-panel-bg)] 合规。
const ARBITRARY_RE = /-\[([^\]]+)\]/g;
const LITERAL_IN_VALUE = /#[0-9a-fA-F]{3,8}\b|\brgba?\(|\d+(?:\.\d+)?(?:px|rem|em|%|vh|vw|pt)\b/;

/** 返回首个违规描述；空串 = 合规 */
function arbitraryViolation(text) {
  for (const m of text.matchAll(ARBITRARY_RE)) {
    const value = m[1];
    if (value.includes("var(--")) continue;
    if (!LITERAL_IN_VALUE.test(value)) continue;
    return `${m[0]} 裸值（任意值只允许 var(--ov-*)）`;
  }
  return "";
}

// 自检：规则必须能抓到裸值、放过 token（守卫静默失效比漏报更危险）
for (const [sample, shouldFail] of [
  ["class=\"p-2 bg-[#fff]\"", true],
  ["class=\"p-[12px]\"", true],
  ["class=\"w-[calc(100%-8px)]\"", true],
  ["class=\"bg-[var(--ov-panel-bg)] text-muted\"", false],
  ["class=\"flex gap-2 rounded-panel\"", false],
]) {
  const caught = arbitraryViolation(sample) !== "";
  if (caught !== shouldFail) fail(`守卫自检失败：${sample}（期望 ${shouldFail ? "违规" : "合规"}）`);
}

const classFiles = [];
(function walk(dir) {
  for (const name of readdirSync(dir)) {
    const p = join(dir, name);
    if (statSync(p).isDirectory()) walk(p);
    else if (/\.(ts|svelte|html)$/.test(p)) classFiles.push(p);
  }
})(srcDir);
for (const f of classFiles) {
  if (f.includes("debug-vite-panel")) continue; // debug 专用，豁免
  const content = readFileSync(f, "utf8").split("\n");
  for (let i = 0; i < content.length; i++) {
    const t = content[i].trim();
    if (t.startsWith("//") || t.startsWith("*") || t.startsWith("/*")) continue;
    const bad = arbitraryViolation(content[i]);
    if (bad) fail(`${f.split(/[\\/]src[\\/]/)[1]}:${i + 1} 类名${bad} — ${t.slice(0, 80)}`);
  }
}

// 3. theme.ts KNOWN_TOKENS 与 :root 定义双向一致（主题应用的写表面 = token 表）
const themeTs = readFileSync(join(srcDir, "theme.ts"), "utf8");
const ktMatch = themeTs.match(/KNOWN_TOKENS\s*=\s*\[([\s\S]*?)\]/);
if (!ktMatch) {
  fail("theme.ts 找不到 KNOWN_TOKENS");
} else {
  const known = [...ktMatch[1].matchAll(/"([a-z0-9-]+)"/g)].map((m) => m[1]);
  const definedNames = [...defined].map((d) => d.replace(/^--ov-/, ""));
  for (const k of known) {
    if (!definedNames.includes(k)) fail(`theme.ts KNOWN_TOKENS 的 ${k} 在 :root 无定义`);
  }
  for (const d of definedNames) {
    if (!known.includes(d)) fail(`:root 的 ${d} 不在 theme.ts KNOWN_TOKENS`);
  }
}

if (failures) {
  console.error(`\n${failures} 处违规`);
  process.exit(1);
}
console.log(`token 守卫通过（${defined.size} 个 --ov-* token 定义在案）`);
