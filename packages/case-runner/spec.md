...
3. **Executor in the binary, concepts in core**: the step executor (sandbox, terminal story state, printing) lives in the ambery-case binary; core carries only parsing / evaluation / observation behind the `case-runner` feature, keeping the library pure.
